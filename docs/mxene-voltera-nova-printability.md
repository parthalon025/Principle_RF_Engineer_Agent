...
|---|---|---|---|
| Shao et al., *Nat. Commun.* 2022 (PMC9184614) | Not stated numerically in main text (small needles used) | **120 µm minimum** demonstrated; "standard" lines 2.5–3+ mm for bulk features | Not determinable from available text |
...
- **Shao et al. (2022):** states thickness scales linearly with number of printed passes/layers and achieved **6260–6900 S/cm conductivity with N=2** (two passes), but does **not** give a numeric per-pass thickness in the accessible text.
...
- **Finest demonstrated MXene extrusion line width: 120 µm** (Shao et al. 2022) — slightly above, not below, the NOVA's advertised 100 µm floor.
...
- The NOVA's own 100 µm figure is explicitly qualified as "dependent on material and nozzle," using Nordson EFD 150–200 µm tips presumably with Voltera's own lower-viscosity, purpose-tuned silver ink (≥10⁷ S/m). MXene paste at the viscosity needed to hold a fine bead shape (~250,000 cP, per Shao et al.) is a different rheological regime than whatever ink the 100 µm spec was measured with, and no source demonstrates sub-100 µm MXene lines on any extrusion platform.
...
|---|---|---|---|---|---|---|
| Pneumatic extrusion (needle) | Not numerically stated | 120 µm min; 3 µm min gap | Not stated (N=2 passes → final result) | 6.26×10⁵ S/m (as-printed) → 6.9×10⁵ S/m (10% RH, 4 h, no heat) | None required; mild humidity conditioning helps | Shao et al., *Nat. Commun.* 13, 3223 (2022), PMC9184614 |
...
5. **Nozzle diameter for the single most relevant analog** (Shao et al. 2022, the pneumatic extrusion dispenser achieving 120 µm lines) is not stated numerically in the accessible main text — only that "smaller needles" were used for the finest lines. Without this number, the line-width-to-nozzle-ID ratio (a key predictor for how a NOVA + Nordson EFD 150/200 µm tip would perform) cannot be computed for the best data point in this report.
