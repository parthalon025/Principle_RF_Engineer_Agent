...
Everything except `titles.json` and `full/…xml` sits unused. `structure/{date}/title-{n}.json` would let the model browse Title 47's tree before spending a possibly-20-MB `full` fetch — today `ingest_fcc_rule` always fetches a whole named part. `versions/title-{n}.json` is this API's real point-in-time mechanism (an `issue_date` range); unused because the module always resolves the current date only — a gap if a design needs what Part 15 said as of a past filing date, not just today's limits. Search (`results`/`suggestions`/`counts/*`) is unused: there is no way today to ask "which Title 47 sections mention EIRP" without already knowing the part. `ancestry`, `agencies.json` and `corrections/title/{n}.json` are also unused; Corrections could flag that an already-ingested rule text was later found erroneous by OFR itself.
