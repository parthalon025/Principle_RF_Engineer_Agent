...
## 14. Two findings about the outside (MXene) literature this loop will be asked to read

Not about #110's own scoring choices — about what fraction of the *evidence* those choices will be applied to actually measures the quantity this document has been discussing.

### 14.1 Shielding effectiveness is not an absorber datum — and that names a gap in the registry, not a reason to discard the evidence

**The tally.** Across two large MXene research passes this programme has run (EM properties; printing methods), roughly **56%** of the headline microwave dB claims are **shielding effectiveness (SE)** — a two-port **transmission** quantity (`SE_T`, built from `S₂₁` via the Schelkunoff `SE_T = SE_R + SE_A (+ SE_M)` decomposition), not reflection loss (from `S₁₁`) and not the absorptivity `A(ω) = 1 − |S₁₁|² − |S₂₁|²` this document has used throughout (§1). It is measured on a different fixture — sample as a barrier between a source and a receiver on the far side — answering "how much gets through," not "how much comes back."

**Why this matters specifically for a skin that prints its own reflector.** ADR-0017 makes this programme's own absorber designs ground-backed by construction: a solid printed reflector sits behind the lossy layer, so transmission through the finished skin is zero **by construction**, not by measurement. For a structure like that, `S₂₁ → 0` and `SE_T → ∞` regardless of how well or badly the front-face loss is tuned — so an SE number carries **no design information** about the one thing this programme's absorber family actually varies. **An SE figure is not a weaker absorber figure — it is the right answer to a question asked on a different fixture, for a structure this programme is not building.**

*In plain terms: these numbers are not bad answers to our question — they are good answers to a question the loop has not been taught to ask.*

**This is a registry gap, not a reason to discard the evidence.** `CONTEXT.md`'s "Intended effect" entry names seven things a requirement can want done to the wave — *"absorbed, reflected in phase, steered, transmitted, scattered diffusely, polarisation-converted, shielded against"* — and ADR-0021 makes all seven patent examples scored candidates on equal footing; Example 3 (the absorber) is this programme's first fixed **test goal**, not its scope, and the charter states plainly that *"No method is privileged."* `shielded against` is therefore a first-class intended effect this repo already names in its own vocabulary — but as of this document, checked directly against `designs/design_families.py`, the registered families are `ABSORBER`, `ABSORBER_TRANSMISSIVE`, `PATCH`, `REFLECTION_PHASE`, `DIFFUSIVE` and `POLARIZATION_CONVERTER`. **There is no `SHIELDING`-shaped family, no `analysis_model`, and no `physical_bound` for it** — the way `ABSORBER` has Rozanov (`docs/absorber-thickness-bandwidth-bound.md`). A candidate whose intended effect is genuinely "block transmission" has, right now, **nowhere in the registry to go** — and the ~56% of outside MXene literature reporting SE is exactly the evidence base such a family would draw on.

**The conclusion is therefore "route it correctly once there is somewhere to route it to," not "discard it."** An SE figure must never be scored against this document's `−10 dB` absorber default (§2) — that comparison is a fixture mismatch, not a weaker result — and until a `shielded against` family exists in the design-family registry (an ADR-0018-shaped open-interface addition, per that ADR's own pattern), this literature should be held as characterised-but-unplaced rather than forced into the absorber column. Whether and how to add that family is a registry question, ticketed separately; this document records the finding, not the decision.

### 14.2 The "absorption ratio" trap: at least three incompatible definitions, and outside sources routinely state none

The phrase *"absorption ratio"* is used in this literature for at least three different quantities, and a source stating a headline percentage without saying which one it means cannot be read at face value.

**Definition 1 — the correct one, and the one this document uses throughout.** `A = 1 − R − T` (power fractions from `|S₁₁|²`, `|S₂₁|²`). A reported *"97.7% absorption ratio"*, read this way with `T ≈ 0` (a ground-backed or otherwise well-isolated structure), gives `R = 1 − 0.977 = 0.023`, i.e. reflectivity `10·log₁₀(0.023) = −16.4 dB` (`CALCULATED`, exact). This is the reading consistent with §1–§2 above, and it would **pass** this repo's own `−10 dB` threshold comfortably.

**Definition 2 — `SE_A/SE_T`, common in the EMI-shielding literature, and a genuinely different quantity.** `SE_R = 10·log₁₀(1/(1−R))` and `SE_A = 10·log₁₀((1−R)/T)` are **decibel-domain** quantities built from the same `R, T`, and a paper reporting *"absorption ratio"* as `SE_A/SE_T` is reporting what *fraction of the total dB attenuation* is attributable to absorption rather than reflection — not what fraction of incident *power* was absorbed. The two are not simply rescaled versions of each other, because the log compresses very different physical situations onto similar-looking ratios. **A real, independently verified example makes this concrete and checkable:** the ACS Omega paper on this exact ambiguity ("Quantitative Interpretation of Electromagnetic Interference Shielding Efficiency: Is It Really a Wave Absorber or a Reflector?", ACS Omega 7(5):4135, 2022, DOI [10.1021/acsomega.1c05657](https://doi.org/10.1021/acsomega.1c05657)) reports, for a pure `Ti₃C₂Tₓ` MXene film 8 µm thick: `SE_T ≈ 58 dB`, `SE_A ≈ 35 dB`, `SE_R ≈ 23 dB`. `CALCULATED`: `SE_A/SE_T = 35/58 = 60.3%` — a number that reads as "absorption-dominated." But converting the same `SE_R = 23 dB` back to a power reflectivity gives `R = 1 − 10^(−23/10) = 0.995` — **99.5% of the incident power is reflected**, and the true `A = 1 − R − T ≈ 0.5%`. A headline "60% absorption ratio" and a physical reality of "99.5% reflected" describe the same sample.

**I could not independently reproduce the brief's illustrative `−16.4 dB` / `−3.66 dB` pair for a single `97.7%` figure read two ways.** Definition 1's `−16.4 dB` is exact and reproduced above. But `SE_A/SE_T` cannot be converted to a reflectivity from the ratio alone — it under-determines `R` and `T` separately without also knowing the *absolute* scale of `SE_T` — and no natural, round assumption for that missing scale reproduced `−3.66 dB` cleanly (values from roughly `−3.5` to `−3.9 dB` came out of nearby assumed scales, but nothing pinned it exactly). This is recorded here rather than smoothed over. The structural point does not depend on matching that specific number: the real, independently checkable `Ti₃C₂Tₓ` example above shows `SE_A/SE_T` and `A = 1 − R − T` diverging **far more dramatically** than a 12.7 dB gap — a `60.3%` "absorption ratio" (which reads as a middling but real absorber) sits alongside a true absorptivity of `0.5%`, i.e. a sample that is, physically, **almost a perfect mirror**. Both readings would in fact fail this repo's `−10 dB` threshold for that specific sample (at `−0.02 dB` and `−4.0 dB` respectively, using each definition's own number as if it were `A`) — so the danger this reading rule guards against is not only that the two definitions can land on opposite sides of a threshold, but that either one, taken alone, can make a device sound like a plausible absorber candidate when it is not one at all.

**A third definition worth naming, since it appears in patent-style absorption-spectrum literature:** `Absorption Ratio [%] = 100 − (S₁₁ magnitude, %) − (S₂₁ magnitude, %)` — summing the *voltage* magnitudes of `S₁₁` and `S₂₁` directly, without squaring them to power. This silently reproduces Definition 1's form while substituting voltage for power, and will read as a *larger* apparent reflectivity than the correct power-based calculation for the same measured device (a full worked comparison was not carried through here, since the qualitative danger — treating a voltage ratio as if it were a power fraction — is the same class of error flagged in §8.1 of `docs/absorber-thickness-bandwidth-bound.md` for the Rozanov bound's own voltage-versus-power trap).

**Recorded as a reading rule:** an outside source's *"absorption ratio"* number is not usable until the definition behind it is known. `SE_A/SE_T` and voltage-magnitude sums are both real, both published, and both incompatible with the `A = 1 − R − T` this document's conventions assume — and the discrepancy is not a rounding error, it is large enough to flip a pass/fail verdict.
