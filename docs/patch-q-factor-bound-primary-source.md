...
2. **It corroborates the vertical-versus-lateral pattern from a second literature.** Assumption 2 lists what the antenna field recognises as bandwidth enhancement: shorting pins, **stacked patches** (a second height — #187's class (b)), and modified ground planes (changing the inductance path). **Lateral detuning of coplanar elements appears nowhere.** `docs/five-paper-absorber-corpus-findings.md` §1 found the same revealed preference across four absorber groups; this is an independent instance in a different field.
