...
| **Zheng, W., Hao, T. & Li, X. (2025)**, "Thickness-to-Bandwidth Limit for Impedance Matching Metasurfaces Design According to the Bode–Fano Theorem," *IEEE Trans. Microwave Theory Tech.* **73**(11):8451–8463, DOI `10.1109/TMTT.2025.3592449` | HTTP fetch of `doi.org` 302-redirects to `ieeexplore.ieee.org/document/11099037/`, which returns no fetchable page content (JS-rendered, subscription-gated). No arXiv preprint or repository copy located (search of the authors' names plus institution, Tongji University, turned up none) | `BANDPASS_FSS`'s `physical_bound` (`designs/design_families.py`) named broadband MATCHING theory — Bode's and Fano's gain-bandwidth limits — as where a search for its passband-bandwidth bound should start, without a citation. This 2025-09 search (2026-09-11) found this one on-point candidate: its own title states a thickness-to-bandwidth limit derived from the Bode–Fano theorem for **transmissive** impedance-matching metasurfaces, closer in shape to a radome's two-port problem than Rozanov's reflective one. Search-engine paraphrase only (not a verbatim quote): the paraphrase describes the metasurface as a homogenised impedance-matching/antireflection layer with an artificial-magnetic-conductor treatment, which leaves open whether the derivation reaches a discrete **periodic aperture array** (this family's actual geometry) or only a continuous layer — unconfirmed either way. Per this table's own MXene-paper entries: no number from this pass should be treated as more than a lead until a human with IEEE Xplore access reads it first-hand | BANDPASS_FSS's physical bound |
