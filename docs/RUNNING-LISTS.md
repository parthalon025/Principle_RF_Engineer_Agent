...
| ~~**IEEE Std 1597.1-2008/2022 & IEEE Std 1597.2-2010** (Feature Selective Validation)~~ | ~~Behind IEEE~~ — **resolved 2026-09-13 (#168), without the standard text itself.** The ADM/FDM/GDM equations, all six grading bins and both GRADE/SPREAD definitions were recovered three independent ways — the FSV authors' own ARMMS paper, their own 1D FSV Tool manual, and Bongiorno & Mariscotti's 2022 open-access paper, which states it transcribes IEEE Std 1597.1 directly — all three agreeing on every constant | ~~The actual standard #110/#168 chose for judging Example 3's reproduction against Landy's curve — read only through consistent secondary-source description~~. Now fully specified in [`feature-selective-validation-spec.md`](./feature-selective-validation-spec.md): the decomposition is a plain **Fourier** split, not wavelet (correcting ADR-0041 point 5); re-running the repo's own `prototype/fsv-feasibility/fsv_prototype.py` with the corrected equations **reverses** its verdict on the disputed patent-vs-Landy pair from `GDM 0.368 "Good"` to `1.018 "Poor"`, answering #116's challenge in FSV's favour | #110, #168, #116 |
...
| **Tretyakov & Simovski (2003)**, "Dynamic model of artificial reactive impedance surfaces," *J. Electromagn. Waves Appl.* **17**(1) 131–145 | Closed access. **Confirmed by two independent indexes that no repository copy exists**: Unpaywall `is_oa:false, has_repository_copy:false, oa_locations:[]`; Semantic Scholar `CLOSED` | Costa's ref [35] — the **original statement of eq (10)**, the thin-spacer capacitance correction. Three things were stranded behind it: (a) whether the prefactor is `2Dε₀/π` (as the 2013 paper prints) or `2Dε₀ε_r/π` (as Costa & Borgese 2021 restate it, citing the same source) — **resolved 2026-09-13 without the original (#234)**, by deriving eq (10) from first principles (a quasi-static Floquet solve of the grid-over-ground problem), which reproduces the equation term-for-term and settles the prefactor as `ε₀ε_r`, corroborated by re-digitising the 2013 paper's own Figure 2(b). **`rf_tools/sheet_impedance.py`'s `COSTA_EQ10_FORM` default should move from `"eps0"` to `"eps0_epsr"`** (separate implementation ticket — not changed here); (b) whether an **inductance** correction was ever written — still open, asserted three times across the Costa papers and published nowhere; (c) eq (10)'s stated validity range in the author's own words — still open. Equation, symbols, derivation and recomputation are in [`costa-thin-spacer-correction.md`](./costa-thin-spacer-correction.md) §10 | #190, #128, #111, #234 |
...
| ~~**Costa, Monorchio & Manara**, *IEEE Antennas and Propagation Magazine* 54(4), 2012, DOI 10.1109/MAP.2012.6309153~~ | **CLOSED-ACCESS CONFIRMED and NO LONGER NEEDED (2026-09-13, #446).** Three independent indexes agree no free copy exists: Unpaywall `is_oa:false, oa_status:closed, has_repository_copy:false, oa_locations:[]`; Semantic Scholar `status:CLOSED`; OpenAlex `is_oa:false, any_repository_has_fulltext:false`. The authors' own institution (University of Pisa) holds the record and reports *"Non ci sono file"* (no files) | **Routed around.** It is the origin of ACES eq (5), the effective-permittivity fit in `x = 10·d/D`, for a dielectric layer above a printed grid — but three later **open-access** papers restate the result: ACES Journal 29(12) 2014 (galley PDF, landing page hides the download link), Borgese & Costa arXiv:2008.00530, and **Costa, IEEE TAP 69(8) 2021** (green OA, `hdl.handle.net/11568/1134368` — found by OpenAlex after Unpaywall and Semantic Scholar both reported it closed), which **generalises eq (5) and states it fails for unequal layer thicknesses** — this repo's own case. Equation, symbol table, limit checks and a digitised cross-check (R² > 0.9998) are in [`costa-dielectric-overlayer-correction.md`](./costa-dielectric-overlayer-correction.md). **T1 (#445) should implement Costa 2021 eqs (9)–(13), not ACES eq (5).** Method note: the repo's "two independent indexes agree" bar for an absence claim should probably be three — OpenAlex found a copy both other indexes missed | #111, #190, #446, #445 |
| **Callaghan, P., Parker, E.A. & Langley, R.J. (1991)**, "Influence of supporting dielectric layers on the transmission properties of frequency selective surfaces," *IEE Proc. H* 138(5):448–454, DOI `10.1049/ip-h-2.1991.0075` | Closed access. Confirmed by two independent indexes that no repository copy exists: Unpaywall `is_oa:false, oa_status:closed, has_repository_copy:false, oa_locations:[]`; Semantic Scholar `status:CLOSED`. Kent Academic Repository holds a record (`kar.kent.ac.uk/38670/`) with no PDF | **Role now scoped by #446, and it is not load-bearing.** Named as a possible second source of the dielectric-overlayer closed form; it is not — the ACES overview cites it only for the thick-substrate approximation's *validity limit*, not a competing formula. What it would still give is that limit in its own authors' words rather than Costa's paraphrase | #446 |
| **De Meulenaere, F. & Van Bladel, J. (1983)**, "Computation of the magnetic polarizability of conducting disks and the electric polarizability of apertures," *IEEE Trans. Antennas Propag.* 31(5), DOI `10.1109/TAP.1983.1143122` | Closed access. Unpaywall confirms `is_oa:false, has_repository_copy:false, oa_locations:[]` — no free copy exists rather than a fetch that failed | The canonical numerical treatment of exactly the quantity `BANDPASS_FSS`'s physical bound needs — static aperture polarizability `γ` — and the most likely published source of independent γ values for **non-square** aperture shapes (circular loop, annulus, ellipse) to validate any future shape added to `rf_tools/aperture_polarizability.py` against. A library login closes it | #546, #547 |
| **Yang, F. & Rahmat-Samii, Y. (2003)**, "Reflection Phase Characterizations of the EBG Ground Plane for Low Profile Wire Antenna Applications," *IEEE Trans. Antennas Propag.* 51(10), 2691–2703, DOI `10.1109/TAP.2003.817559` | Closed access. Confirmed by two independent indexes that no repository copy exists: Unpaywall `is_oa:false, oa_status:"closed", has_repository_copy:false, oa_locations:[]`; OpenAlex `is_oa:false, oa_status:"closed", any_repository_has_fulltext:false`. Xplore's own PDF path returns HTTP 418 | **No longer load-bearing for the ±90° in-phase-reflection convention (#471)** — that is now read first-hand from Sievenpiper et al. (1999) p. 2064, retrieved in full via the Internet Archive's copy of co-author Eli Yablonovitch's Berkeley group page; see [`amc-magnetic-mirror-evidence-review.md`](./amc-magnetic-mirror-evidence-review.md) §1. What is still stranded is this paper's own, narrower subject: how much of the ±90° *surface* band a real low-profile dipole actually keeps | #471, #465, #220 |
| **Saleem, M. & Li, X.-L. (2020)**, "Low Scattering Microstrip Antenna Based on Broadband Artificial Magnetic Conductor Structure," *Materials* 13(3):750, DOI `10.3390/ma13030750` | MDPI HTTP 403. Its PMC mirror (`PMC7040665`), which normally routes around MDPI, serves an HTML shell rather than the PDF | **The highest-value unfetched item for `REFLECTION_PHASE` (#471).** Reported shape is exactly the programme's gap: two metal layers, **no vias**, FR-4 εr 4.3, 2.4 mm thick, 8 mm period, **0.20 mm smallest gap — this shop's exact feature floor** — with an in-phase band spanning X-band. If it reads as advertised it is simultaneously the missing X-band measured number, a via-less design, and a design at our floor. A retrieval summary's arithmetic did not close (4.6 GHz over an 8.7 GHz centre is 52.9%, not the 42% quoted alongside it) and conflated the ±90° AMC band with the unrelated 180°±37° checkerboard criterion, so nothing from it is carried as a result yet | #471, #465, #220 |
| **Zhang et al. (2019)**, the primary source behind the "90.5% accuracy at 2° phase error" meta-atom-phase ML surrogate figure (`docs/ai-metasurface-survey-against-the-seven-examples.md` §4) | Closed access (Wiley). Confirmed no free copy exists anywhere | **Corrected 2026-09-13 (#202) from the publisher-deposited abstract alone** (full text unreachable): the figure is **90.05%**, not 90.5%, and 2° is a **tolerance window a prediction must land inside**, not an error magnitude — roughly 10% of predictions miss by an unpublished amount, which is the actual unresolved number. See [`ml-phase-surrogate-for-example7-review.md`](./ml-phase-surrogate-for-example7-review.md) | #202 |
...
4. **"MXene behaves as a copper ~85× lossier" is a DC ratio applied at 10 GHz.**
   ...
   of magnitude less than every comparison had assumed. **Narrowed further,
   2026-09-13 (#454, §3 item 33):** this single ratio was computed from the
   one conductivity value on hand; four RF-band measured values now exist,
   and the penalty is a **4.1×–7.0× copper / 1.7×–2.9× silver range**, not
   one number — see [`mxene-rf-conductivity-measurement-review.md`](./mxene-rf-conductivity-measurement-review.md).
...
33. **Correction 7 and #111's thickness correction disagree about MXene's skin
    ...
    figures on the map). Note both routes still converge on #128's finding
    that for a *lossy* element the aspect ratio, not the film thickness, is the
    knob that reaches the target — so this was a live discrepancy about MXene,
    not a threat to that conclusion.

    **RESOLVED 2026-09-13 (#454), and the resolution overtakes both sides.**
    `docs/mxene-rf-conductivity-measurement-review.md` §3 reconstructed the
    33 µm figure as a five-skin-depth estimate at the bottom of the band
    divided by three (`INFERRED`), leaving both sides on the **same**
    σ ≈ 6.9×10⁵ S/m — the two implied conductivities recompute to
    6.895×10⁵ and 2.093×10⁵ S/m. **Four RF-band measurements now sit above
    both positions, not between them**, so they adjudicate by overtaking
    rather than splitting the difference: σ spans 1.2×10⁶–3.5×10⁶ S/m
    (AlHassoon 2020; Rakhmanov 2023; Han 2021, DOI 10.1002/adma.202003225;
    Tajin & Dandekar 2022, DOI 10.1109/ACCESS.2022.3154038). **Correction 7
    is strengthened, not weakened** — a 10 µm film is 2.18 skin depths at the
    lowest measured RF conductivity and **3.72 skin depths** at the highest,
    past the three-skin-depth "thickness is a dead knob" rule with margin.
    **The map's 33 µm/3δ position does not survive**: 3δ at 10 GHz is
    13.8 µm at the lowest measured RF σ and **8.1 µm** at the highest — both
    inside the 20–35 µm a printed conductor actually reaches, not above it.
    `CALCULATED`.
