...
**2. This repo's own curvature formula gets the sign right for one axis and is roughly 7–9× too large in magnitude.** `rf_tools/calculations.py` predicts `f_curved = f_flat · L/(2R·sin(L/2R))`, i.e. `Δf/f ≈ L²/(24R²)`. For Boeykens' prototype 3 (resonant length 69.3 mm) at R = 31.5 mm that is **+20.2%**; the measured/modelled cylindrical cavity gives **+2.6–3.0%**. The formula also has no substrate thickness in it, and applies the same correction to *both* bend axes where the cavity model gives zero for one of them. §1.5.
...
### 1.5 What this says about `rf_tools/calculations.py`
...
Note the sanity check against §3.2: the two "still works" datapoints sit inside this. Su et al.'s MXene coding surface on R = 100 mm; Zhang et al.'s THz surface on a full cylinder of R = 5 mm ≈ 17λ, where the *illuminated* arc is what matters, not the closed circumference.
