...
@function_tool(strict_mode=False)  # same rationale as generate_freecad_curved_geometry
# above -- `primitives`/`curvature`'s shape does not fit the SDK's strict-schema
# requirement.
...
ROLE_SPECS: list[RoleSpec] = [
    ...
    RoleSpec(
        ...
        domain_note=(
            ...
            "an arbitrarily tilted plane (see geometry/freecad_curved.py). Also gets "
            "run_freecad_fem_mesh_geometry (issue #288) for when that curved solid "
            "needs to go into a real FEM/mesh-based solver (Elmer, OpenParEM) rather "
            "than just a STEP file -- drives FreeCAD's own FEM workbench meshing "
            "instead of stopping at export. Also "
