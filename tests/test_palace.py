...
# The conductivity-sheet tests below never set geometry["pec_patches"], so
# their boundary attribute always starts right after BOUND_PEC_START (issue
# #289's _conductivity_boundary_base is BOUND_PEC_START + len(pec_patches)).
CONDUCTIVITY_BASE_NO_PEC_PATCHES = _conductivity_boundary_base([])
...
def _sheet_boundary_faces(mesh_text: str, num_boundary_faces: int) -> list[list[str]]:
    ...
    return [
        ...
        if int(line.split()[0]) == CONDUCTIVITY_BASE_NO_PEC_PATCHES
    ]
...
def test_generate_palace_config_emits_conductivity_boundary():
    ...
    assert conductivity == [
        {
            "Attributes": [CONDUCTIVITY_BASE_NO_PEC_PATCHES],
            "Conductivity": 2.0e5,
            "Permeability": 1.0,
            "Thickness": 5e-6,
        }
...
def test_generate_palace_config_multiple_conductivity_sheets_get_distinct_attributes():
    ...
    assert [c["Attributes"] for c in conductivity] == [
        [CONDUCTIVITY_BASE_NO_PEC_PATCHES],
        [CONDUCTIVITY_BASE_NO_PEC_PATCHES + 1],
    ]
