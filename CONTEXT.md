...
- **Verification corpus**: a fixed set of **golden queries** — a query
  paired with the specific result it must retrieve or the value it must
  reproduce — run as an ordinary test with a pass/fail threshold, so a
  regression in retrieval or a simulator adapter fails `uv run pytest`
  instead of surfacing later as a silently wrong answer. Two independent
  instances so far: `tests/test_verification_corpus.py`'s golden queries
  for `search_knowledge` (recall@5 and zero-result rate over
  `knowledge/corpus/`, lexical-only — no embedding credential is
  configured in this environment); and MEEP's role as an
  independent-method cross-check against openEMS (`simulation/meep.py`).
  A simulator-adapter golden corpus validated against independently-
  sourced literature values (not against `rf_tools/calculations.py`
  itself, which would let a shared bug in both agree and pass silently)
  does not exist yet.
  _Avoid_: conflating with **Verification item** above — that's a specific
  design's per-requirement pass/fail row in `verification_items`, scoped to
  one design; a verification corpus is a repo-wide regression test with no
  `design_id`, run against the codebase itself rather than one design's
  results.
