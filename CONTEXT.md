...
- **Inputs**: plain numeric parameters for calculations; local Touchstone
  ...
  openEMS/Palace shell out to the real tool via `subprocess`; HFSS is
  implemented but intentionally confined to running only on a controlled,
  licensed AEDT workstation — see `simulation/hfss.py`). Palace
  (`simulation/palace.py`) is the only adapter with native
  Floquet/periodic-boundary ports, for characterizing a periodic
