...
_ALIASES: dict[str, str] = {
    ...
    # text -- it holds only for a surface that REDISTRIBUTES rather than
    # dissipates, and only above a supercell period of sqrt(2)*lambda. A
    # coding checkerboard whose cells are also lossy voids the first
    # precondition, and ADR-0047's own table assigns AMC-checkerboard
    # backscatter reduction to Gustafsson & Sjoberg, which REFLECTION_PHASE
    # carries. Re-routing the alias is a DECISION, ticketed at #452, and
    # ADR-0050 does not take it: neither family added there is a backscatter
    # mechanism, so nothing about the two new members changes what the right
    # routing would be.
