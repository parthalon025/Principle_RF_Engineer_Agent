...
BANDPASS_FSS = DesignFamily(
    ...
    physical_bound=UnreadPhysicalBound(
        ...
        citation=(
            "Searched 2026-09-11 (see docs/RUNNING-LISTS.md section 1); still "
            "recorded as unread, not NO_PHYSICAL_BOUND, on the same rule "
            "ABSORBER_TRANSMISSIVE and POLARIZATION_CONVERTER use: absence of a "
            "read primary source is not evidence that no bound exists. What IS "
            "settled is that Rozanov does not apply, for the same reason it does "
            "not apply to ABSORBER_TRANSMISSIVE -- its derivation opens by fixing "
            "a slab 'overlying a perfectly reflecting plane' "
            "(docs/rozanov-bound-primary-source.md section 3, assumption (b)) -- "
            "and that ADR-0047's per-family table does not cover a transmissive "
            "passband at all. The lead this raised for -- broadband MATCHING "
            "theory, Bode's and Fano's gain-bandwidth limits -- turned up one "
            "on-point candidate: Zheng, Hao & Li, 'Thickness-to-Bandwidth Limit "
            "for Impedance Matching Metasurfaces Design According to the "
            "Bode-Fano Theorem', IEEE Trans. Microwave Theory Tech. 73(11):"
            "8451-8463, 2025, doi:10.1109/TMTT.2025.3592449 -- which states in "
            "its own title that it derives a thickness-to-bandwidth limit for "
            "TRANSMISSIVE impedance-matching metasurfaces, the closer shape to "
            "this family's two-port problem than Rozanov's reflective one. It is "
            "STRANDED: IEEE Xplore returns no fetchable content here, so the "
            "above rests on a search-engine paraphrase of the abstract, never a "
            "verbatim quote, and per this repo's own citation-is-not-derivation "
            "rule (docs/RUNNING-LISTS.md's MXene-paper entries) nothing in it may "
            "be treated as confirmed. In particular, whether its bound reaches a "
            "discrete PERIODIC APERTURE array -- this family's actual geometry -- "
            "rather than a continuous homogenised impedance-matching layer is "
            "itself unread. NOT a claim that it applies; a sharper lead than "
            "'nobody has searched', nothing more, until somebody with IEEE "
            "access reads it."
        ),
    ),
)
