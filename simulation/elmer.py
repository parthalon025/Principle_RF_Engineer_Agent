...
class ElmerSimulator(Simulator):
    ...
    def run(self, job: dict) -> SimulationResult:
        ...
        try:
            ...
            raise SimulatorError(f"ElmerSolver timed out after {timeout_s}s: {exc}") from exc
...
def _face_bbox_selector(p1_m: list[float], p2_m: list[float], face: str, eps: float) -> str:
...
def _parse_save_scalars(workdir: Path, filename: str = "scalar_values.dat") -> dict[str, Any]:
    ...
    try:
        header_idx = next(i for i, line in enumerate(names_lines) if "Variables in columns" in line)
